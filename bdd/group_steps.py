...
@given('group with <name>, <header> and <footer>', target_fixture="new_group")
...
def verify_modify_group(app, random_group):
    app.group.modify_group_by_id(random_group.id, random_group)


@then('the new group list is equal to the old group with the modify group')
def verify_group_modified(db, non_empty_group_list, app, check_ui):
    old_groups = non_empty_group_list
    new_groups = db.get_groups_list()
    for i in old_groups:
        if i.id == id:
